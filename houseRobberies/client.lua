...
local robbableItems = {
 [1] = {x = 1.90339700, y = -3.80026800, z = 1.29917900, name = "Buzdolabı", isSearched = false},
 [2] = {x = -3.50363200, y = -6.55289400, z = 1.30625800, name = "Çekmece", isSearched = false},
 [3] = {x = -3.50712600, y = -4.13621600, z = 1.29625800, name = "Masa", isSearched = false},
 [4] = {x = 8.47819500, y = -2.50979300, z = 1.19712300, name = "Masa", isSearched = false},
 [5] = {x = 9.75982700, y = -1.35874100, z = 1.29625800, name = "Depo", isSearched = false},
 [6] = {x = 8.46626300, y = 4.53223600, z = 1.19425800, name = "Dolap", isSearched = false},
 [7] = {x = 5.84416200, y = 2.57377400, z = 1.22089100, name = "Masa", isSearched = false},
 [8] = {x = 3.04164100, y = 0.31671810, z = 3.58363900, name = "Gümüş Kutusu", isSearched = false},
 [9] = {x = 6.86376900, y = 1.20651200, z = 1.36589100, name = "Yatak Altı", isSearched = false},
 [10] = {x = 2.03442400, y = -5.61585100, z = 3.30395600, name = "Cupboards", isSearched = false},
 [11] = {x = -5.53120400, y = 0.76299670, z = 1.77236000, name = "Televizyon", isSearched = false},
 [12] = {x = -1.24716200, y = 1.07820500, z = 1.69089300, name = "Kahve Masası", isSearched = false},
 [13] = {x = 2.91325400, y = -4.2783510, z = 1.82746400, name = "Masa", isSearched = false},
}
 ...
 if isNight() and not isRobbing then
  ...
  for id,v in pairs(robbableHouses) do
   if GetDistanceBetweenCoords(playerCoords, v.x, v.y, v.z, true) <= 2.5 then
    ...
    local finished = exports["xhen-lockpicking"]:lockpick(100,5,2,10)
    if finished == 100 then
      TriggerEvent('lockpickAnimation')
      FreezeEntityPosition(PlayerPedId(), true)
      local sa = exports["np-taskbar"]:taskBar(3000,"Ev Maymuncuklanıyor")
      if (sa == 100) then
        ...
        ExecuteCommand('e c')
        TriggerEvent('notification', 'Başaramadık Abi', 2)
        FreezeEntityPosition(PlayerPedId(), false)
        ClearPedTasks(PlayerPedId())
        Citizen.Wait(1)
        ClearPedTasks(PlayerPedId())
  ...
  elseif not isRobbing then
    ClearPedTasks(PlayerPedId())
    TriggerEvent('notification', 'Saat çok erken ', 2)
  ...
  if isRobbing then
   for i=1,#myRobbableItems do
    if (GetDistanceBetweenCoords(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], GetEntityCoords(GetPlayerPed(-1))) < 1.4) and not myRobbableItems[i]['isSearched'] then
     DrawText3D(generator.x + myRobbableItems[i]["x"], generator.y + myRobbableItems[i]["y"], generator.z + myRobbableItems[i]["z"], '~w~Press ~g~H~s~ To Search ' .. myRobbableItems[i]["name"])
     ...
     if IsControlJustReleased(1, 74) then
      ...
      local test = exports["np-taskbar"]:taskBar(5000," " ..myRobbableItems[i]['name'].." Arıyorsun")
      if (test == 100) then
...
Citizen.CreateThread(function()
  ...
  local pos = GetEntityCoords(ped)
  local distance = GetDistanceBetweenCoords(pos.x,pos.y,pos.z,safepos.x,safepos.y,safepos.z,false)
      if distance <= 1.2 then
          DrawText3D(safepos.x, safepos.y,safepos.z+1.2, "[E]")
          if IsControlJustReleased(0, 86) then
            TriggerEvent("safecracking:loop",5)
            Citizen.Wait(500000000000000000000)    
          end
      end
      Citizen.Wait(5)
  end
